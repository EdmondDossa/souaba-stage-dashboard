...
export default function ReservationPageValide ({ }) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "Arrivée":
        return "bg-primary text-black font-medium";
      case "Présent":
  ...
  return (
    <div className="flex-1 bg-white border-white p-7 rounded min-h-screen mt-3">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Liste des réservation validées</h1>
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-primary text-black rounded-lg px-3 py-1.5 text-sm hover:bg-primary/80">
          ...
          <tbody>
            {reservations.map((res) => (
              <tr key={res.id} className="border-b border-gray-100 hover:bg-gray-50 py-10">
                <td className="p-3 text-black font-bold">{res.name}</td>
                <td className=" text-black font-bold">{res.id}</td>
                <td className=" text-black font-bold">{res.type}</td>
                <td className=" text-black font-bold">{res.room}</td>
                ...
                <td className="p-3 text-right flex justify-center gap-2">   
                  ...
                  <button className="flex items-center gap-1 bg-blue-500 text-white text-xs px-1 rounded hover:bg-blue-600">
                    Payer
                  </button>
                </td>
              </tr>
        ...
        <div className="flex justify-between items-center p-4 text-l text-gray-500 bg-gray-50">
          ...
          <div className="flex gap-1 text-black justify-end">
            {[1, 2, 3, "...", 8].map((num, i) => (
              <button
                key={i}
                className={`px-3 py-1 m-1 text-black text-l rounded ${
                  num === 1
