...
export default function FactureModal({ show, onClose, data }) {
    ...
    const [showConfirm, setShowConfirm] = useState(false); // État local pour gérer l'affichage de la modale de confirmation
    const [showFacture, setShowFacture] = useState(true); // État local pour gérer l'affichage de la modale facture


    if (!show && !showConfirm) return null; // Ne rien rendre si aucune des modales n'est affichée
    ...
    return (
        <>
           {/* MODALE FACTURE */}
            {showFacture && (
                <div
                    className="flex gap-3 fixed rounded-lg inset-0 z-50 items-center justify-center bg-[#00000033] transition-opacity duration-200"
                ...
                >
                    <div className="bg-[#FFFFFF] rounded shadow-2xl max-w-md w-full relative overflow-hidden transform transition-all duration-300 scale-100">
                        ...
                        <button
                            onClick={onClose}
                            className="absolute top-5 right-5 text-[#FFFFFF] hover:scale-110 transition-transform"
                        >
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                <button className="bg-[#D70A0A] text-white px-4 py-2 rounded-md text-sm hover:bg-[#b00808] transition-colors">
                                    Imprimez
                                </button>

                                <button
                                    className="bg-[#29B06F] text-white px-4 py-2 rounded-md text-sm hover:bg-[#248F5E] transition-colors"
