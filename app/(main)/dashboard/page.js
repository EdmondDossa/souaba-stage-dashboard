  ...
  useEffect(() => {
    const fetchKPIData = async () => {
      try {
        ...
        const reservations = Array.isArray(reservationsRes.data?.data)
        ...
        const thisWeekReservations = reservations.filter((r) => {
          ...
          return (
            createdAt && createdAt >= thisWeekStart && createdAt <= thisWeekEnd
          );
        });
        ...
        const lastWeekReservations = reservations.filter((r) => {
          const createdAt = getDate(r.createdAt);
          return (
            createdAt && createdAt >= lastWeekStart && createdAt <= lastWeekEnd
          );
        });
        ...
        const thisWeekCheckOuts = reservations.filter((r) => {
          ...
          return (
            checkOut &&
            checkOut >= thisWeekStart &&
            checkOut <= thisWeekEnd &&
            !["CANCELLED", "NO_SHOW"].includes(r.status)
          );
        });
        ...
        const lastWeekCheckOuts = reservations.filter((r) => {
          ...
          return (
            checkOut &&
            checkOut >= lastWeekStart &&
            checkOut <= lastWeekEnd &&
            !["CANCELLED", "NO_SHOW"].includes(r.status)
          );
        });
        ...
        setKpiData({
          ...
          checkOuts: {
            ...
            trend: calculateTrend(
              thisWeekCheckOuts.length,
              lastWeekCheckOuts.length
            ),
          },
          totalRevenue: {
